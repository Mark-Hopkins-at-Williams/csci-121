...
def is_favorite(root, lbl):
    return label(insertion_point(root, lbl)) == lbl


def insert(root, lbl):
    node = root
    place_to_insert = insertion_point(node, lbl)
    if place_to_insert != None:
        if lbl < label(place_to_insert):
            set_left(place_to_insert, leaf(lbl))
        elif lbl > label(place_to_insert):
            set_right(place_to_insert, leaf(lbl))
